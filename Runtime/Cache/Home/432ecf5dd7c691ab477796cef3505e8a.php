...
<?php if(!empty($normal_tips)): ?><p class="normal_tips"><b class="fa fa-info-circle"></b> <?php echo ($normal_tips); ?></p><?php endif; ?> 
      <?php if($add_button || $del_button || $search_button): ?><div class="table-bar">
        <div class="fl">
          <?php if(empty($model["extend"])): ?><div class="tools">
                ...
                <?php if(is_array($top_more_button)): $i = 0; $__LIST__ = $top_more_button;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><a class="btn" href="<?php echo ($vo["url"]); ?>"><?php echo ($vo["title"]); ?></a><?php endforeach; endif; else: echo "" ;endif; ?>            
			</div><?php endif; ?>
        </div>
        <!-- 高级搜索 -->
        <?php if($search_button): ?><div class="search-form fr cf">
          <div class="sleft">
            ...
            <input type="text" name="<?php echo ((isset($model['search_key']) && ($model['search_key'] !== ""))?($model['search_key']):'title'); ?>" class="search-input" value="<?php echo I('title');?>" placeholder="<?php echo ($placeholder); ?>">
      ...
      <div class="data-table">
        <div class="table-striped">
          <?php if(is_array($list_data)): $i = 0; $__LIST__ = $list_data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$data): $mod = ($i % 2 );++$i;?><div class="hotel-list">
              <div class="left">
                <img src="<?php echo (get_cover_url($data["image"])); ?>" >
                <h3><?php echo ($data["name"]); ?></h3>
              </div>
              <div class="right">
                <div class="icons">
                  <a class="c4" href="<?php echo U('orders?hid='.$data['id']);?>"><i class="fa fa-shopping-cart"></i><br>订单管理</a>
                  <a class="c2" href="<?php echo U('edit?model='.$model['id'].'&id='.$data['id']);?>"><i class="fa fa-edit"></i><br>酒店设置</a>
                  <a class="c1" href="<?php echo U('roomtype?hid='.$data['id']);?>"><i class="fa fa-cny"></i><br>房型与价格</a>
                  <a class="c3" href="<?php echo U('room?hid='.$data['id']);?>"><i class="fa fa-key"></i><br>房间管理</a>
                  <!-- <a class="c5" href="<?php echo U('images?hid='.$data['id']);?>"><i class="fa fa-picture-o"></i><br>图片展示</a> -->
                </div>
                <div class="hotel-info">
                  <span><i class="fa fa-phone"></i><?php echo ($data["tel"]); ?></span>
                  <span><i class="fa fa-home"></i><?php echo ($data["address"]); ?></span>
                  <span><a class="confirm ajax-get" href="<?php echo U('del?model='.$model['id'].'&id='.$data['id']);?>"><i class="fa fa-times"></i>删除酒店</a></span>                  
                </div>
                </ul>
              </div>
              <div class="clear"></div>
