                      ...
                      <?php switch($field["type"]): case "num": ?><input type="number" class="text input-medium" name="<?php echo ($field["name"]); ?>" value="<?php echo I($field[name], $field[value]);?>"><?php break;?>
                        ...
                        <?php case "picture": ?><div class="controls uploadrow" title="点击修改图片">
                            ...
                            <div class="upload-img-box">
                              <?php if(!empty($data[$field['name']])): ?><div class="upload-pre-item"><img width="120" height="120" src="/weiphp<?php echo (get_cover($data[$field['name']],'path')); ?>"/></div><?php endif; ?>
                            </div>
                          </div>
                          <script type="text/javascript">
								//上传图片
								...
								function uploadPicture<?php echo ($field["name"]); ?>(file, data){
							        ...
							        if(data.status){
							        	...
							        	$("#cover_id_<?php echo ($field["name"]); ?>").parent().find('.upload-img-box').show().html(
...
<div class="controls">
	<table id="option_list" class="add_list_table" cellpadding="0" cellspacing="1">
      ...
      <?php if(is_array($option_list)): $i = 0; $__LIST__ = $option_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="add_list">
        ...
        <div class="upload-img-box">
          <?php if(!empty($vo["image"])): ?><div class="upload-pre-item"><img width="120" height="120" src="/weiphp<?php echo (get_cover($vo["image"],'path')); ?>"/></div><?php endif; ?>
        </div>
        </div>
        </td>
        <td>
